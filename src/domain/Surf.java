...
import jopensurf.FastHessian;
import jopensurf.GaussianConstants;
import jopensurf.ImageTransformUtils;
...
public class Surf {
        ...
        public IntegralImage getIntegralImage(){
                if ( mIntegralImage == null ){
                        mIntegralImage = new IntegralImage(ImageTransformUtils.convertToGrayscale(mOriginalImage));
                }
...
//      }
        public static void main(String args[]){
                try {   ImageProcessingImpl ip=new ImageProcessingImpl();
                        BufferedImage image = ImageIO.read(new File("C:\\Avto\\Volkswagen-Golf-VII-8.jpg"));
                        image=ip.trancateAreaFromImage(image, 150, 150);
                        Surf board = new Surf(image);
                        List<SURFInterestPoint> points = board.getFreeOrientedInterestPoints();
                        System.out.println("Found " + points.size() + " interest points");
                        
                         image = ImageIO.read(new File("C:\\Emb\\Volkswagen.jpg"));
                      Surf board5 = new Surf(image);
                      Map<SURFInterestPoint,SURFInterestPoint> matchingPoints =  board.getMatchingPoints(board5,false);
                      System.out.println("between the first and third image there are " + matchingPoints.size() + " of " + board.getFreeOrientedInterestPoints().size() + " possible matching points");
